...
function Diction(props) {
  ...
  return (
    <div className='container py-2 px-2 my-4 row'>
      ...
      <div className='col-5'>
      <div className='text-center my-2' style={props.mode==='light'?{color:'black'}:{color:'white'}}><b>Confused in a word? No problem search it below</b></div>
        ...
        <button onClick={show} className="form-control btn btn-lg btn-primary my-3">Show</button>
        </div>
        <div className='col-5'>
            <textarea cols={100} rows={8} id='define'style={props.mode==='light'?{color:'black',backgroundColor:'white',border:"2px solid black"}:{color:'black',backgroundColor:'gray',border:"2px solid black",cursor:"default"}}></textarea></div>
    </div>
  )
}
